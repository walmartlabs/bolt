...
var cpFiles = function cpFiles() {
  fs.copy("node_modules/electrode-bolt/templates", path.join(process.cwd(), "."), function (err) {
    ...
    ["src/components/boilerplate-component.jsx", "test/client/spec/components/boilerplate-component.spec.jsx"].forEach(function (filename) {
...
var updatePackageJSON = function updatePackageJSON() {
  ...
  for (task in tasks) {
    ...
    pkg.scripts[task] = isBak ? "echo \"original version of " + task + " is now " + task + ":bak" + " if you believe this is in error, please remove this task and change " + task + ":bak back to " + task + "\" && " + tasks[task] : tasks[task];
  }
  ...
  jsonfile.writeFile(path.join(process.cwd(), "package.json"), pkg, function (err, file) {
    if (err) {
      throw err;
    }
  });
